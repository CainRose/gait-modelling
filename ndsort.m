...
%--------------------------------------------------------------------------
% Murtaza Bohra - code change here, to rank members aggressively according
% to product of their objective function values...
N = length(pop);
...
% 2. Sorting
product_obj = zeros(1, N);
% keyboard();
for i = 1:N
    product_obj(i) = (pop(i).obj(1));%*(pop(i).obj(2))*(pop(i).obj(3));
end
[product_sorted, sorting_idx] = sort(product_obj);
sorted_pop = pop;

for j = 1:N
    sorted_pop(j) = pop(sorting_idx(j));
    sorted_pop(j).rank = j;
end
